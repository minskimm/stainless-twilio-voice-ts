...
import { ArchiveDeleteCallParams, Archives } from './resources/archives';
import {
  ByocTrunk,
...
import {
  ...
  SourceIPMappingListParams,
  SourceIPMappingListResponse,
  SourceIPMappingUpdateParams,
  SourceIPMappings,
} from './resources/source-ip-mappings';
...
import {
  ConnectionPolicies,
  ConnectionPolicy,
  ConnectionPolicyCreateParams,
  ConnectionPolicyListParams,
  ConnectionPolicyListResponse,
  ConnectionPolicyUpdateParams,
} from './resources/connection-policies/connection-policies';
import {
...
export interface ClientOptions {
  /**
   * Defaults to process.env['TWILIO_VOICE_OPENAPI_USERNAME'].
   */
  username?: string | undefined;

  /**
   * Defaults to process.env['TWILIO_VOICE_OPENAPI_PASSWORD'].
   */
   ...
   *
   * Defaults to process.env['TWILIO_VOICE_OPENAPI_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['TWILIO_VOICE_OPENAPI_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Twilio Voice OpenAPI API.
 */
export class TwilioVoiceOpenAPI {
  username: string;
  password: string;
   ...
   * API Client for interfacing with the Twilio Voice OpenAPI API.
   *
   * @param {string | undefined} [opts.username=process.env['TWILIO_VOICE_OPENAPI_USERNAME'] ?? undefined]
   * @param {string | undefined} [opts.password=process.env['TWILIO_VOICE_OPENAPI_PASSWORD'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['TWILIO_VOICE_OPENAPI_BASE_URL'] ?? https://voice.twilio.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('TWILIO_VOICE_OPENAPI_BASE_URL'),
    username = readEnv('TWILIO_VOICE_OPENAPI_USERNAME'),
    password = readEnv('TWILIO_VOICE_OPENAPI_PASSWORD'),
    ...opts
  }: ClientOptions = {}) {
    if (username === undefined) {
      throw new Errors.TwilioVoiceOpenAPIError(
        "The TWILIO_VOICE_OPENAPI_USERNAME environment variable is missing or empty; either provide it, or instantiate the TwilioVoiceOpenAPI client with an username option, like new TwilioVoiceOpenAPI({ username: 'My Username' }).",
      );
    }
    if (password === undefined) {
      throw new Errors.TwilioVoiceOpenAPIError(
        "The TWILIO_VOICE_OPENAPI_PASSWORD environment variable is missing or empty; either provide it, or instantiate the TwilioVoiceOpenAPI client with an password option, like new TwilioVoiceOpenAPI({ password: 'My Password' }).",
      );
    }
    ...
    const options: ClientOptions = {
      ...
      password,
      ...opts,
      baseURL: baseURL || `https://voice.twilio.com`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? TwilioVoiceOpenAPI.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('TWILIO_VOICE_OPENAPI_LOG'), "process.env['TWILIO_VOICE_OPENAPI_LOG']", this) ??
      defaultLogLevel;
  ...
  static TwilioVoiceOpenAPI = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static TwilioVoiceOpenAPIError = Errors.TwilioVoiceOpenAPIError;
  ...
  byocTrunks: API.ByocTrunks = new API.ByocTrunks(this);
  connectionPolicies: API.ConnectionPolicies = new API.ConnectionPolicies(this);
  dialingPermissions: API.DialingPermissions = new API.DialingPermissions(this);
  settings: API.Settings = new API.Settings(this);
  ipRecords: API.IPRecords = new API.IPRecords(this);
  sourceIPMappings: API.SourceIPMappings = new API.SourceIPMappings(this);
}
...
export declare namespace TwilioVoiceOpenAPI {
  export type RequestOptions = Opts.RequestOptions;

  export { Archives as Archives, type ArchiveDeleteCallParams as ArchiveDeleteCallParams };

  export {
    ByocTrunks as ByocTrunks,
  ...
  export {
    SourceIPMappings as SourceIPMappings,
