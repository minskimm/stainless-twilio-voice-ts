...
import { APIPromise } from 'stainless-twilio-voice/core/api-promise';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['TWILIO_VOICE_OPENAPI_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: TwilioVoiceOpenAPI) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new TwilioVoiceOpenAPI({ username: 'My Username', password: 'My Password' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['TWILIO_VOICE_OPENAPI_LOG'] = 'debug';
      const client = new TwilioVoiceOpenAPI({
        logger: logger,
        username: 'My Username',
        password: 'My Password',
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['TWILIO_VOICE_OPENAPI_LOG'] = 'not a log level';
      const client = new TwilioVoiceOpenAPI({
        logger: logger,
        username: 'My Username',
        password: 'My Password',
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'TWILIO_VOICE_OPENAPI_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['TWILIO_VOICE_OPENAPI_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['TWILIO_VOICE_OPENAPI_BASE_URL'] = 'https://example.com/from_env';
      const client = new TwilioVoiceOpenAPI({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['TWILIO_VOICE_OPENAPI_BASE_URL'] = ''; // empty
      const client = new TwilioVoiceOpenAPI({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://voice.twilio.com');
    });

    test('blank env variable', () => {
      process.env['TWILIO_VOICE_OPENAPI_BASE_URL'] = '  '; // blank
      const client = new TwilioVoiceOpenAPI({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://voice.twilio.com');
    });
  });
  ...
  test('maxRetries option is correctly set', () => {
    ...
    // default
    const client2 = new TwilioVoiceOpenAPI({ username: 'My Username', password: 'My Password' });
    expect(client2.maxRetries).toEqual(2);
  });

  test('with environment variable arguments', () => {
    // set options via env var
    process.env['TWILIO_VOICE_OPENAPI_USERNAME'] = 'My Username';
    process.env['TWILIO_VOICE_OPENAPI_PASSWORD'] = 'My Password';
    const client = new TwilioVoiceOpenAPI();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['TWILIO_VOICE_OPENAPI_USERNAME'] = 'another My Username';
    process.env['TWILIO_VOICE_OPENAPI_PASSWORD'] = 'another My Password';
    const client = new TwilioVoiceOpenAPI({ username: 'My Username', password: 'My Password' });
    expect(client.username).toBe('My Username');
    expect(client.password).toBe('My Password');
  });
